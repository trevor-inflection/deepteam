...
def get_vulnerability_from_args(vul_type: str, subtype: str) -> Optional[VulnerabilityType]:
...
def test_local_baseline_attacks(purpose: str):
    ...
    print(f"Testing local baseline attack generation with OpenAI for purpose: {purpose}\n")
...
def test_specific_vulnerability(vul_type: VulnerabilityType, max_goldens: int, purpose: str):
    ...
    for type_class, template_class in AttackSimulatorTemplate.TEMPLATE_MAP.items():
        if isinstance(vul_type, type_class):
            prompt_template = template_class.generate_baseline_attacks(vul_type, max_goldens, purpose)
...
def simulate_local_attacks(vul_type: str, subtype: str, purpose: str):
    ...
    try:
        attacks = simulator.simulate_local_attack(purpose, vulnerability, MAX_GOLDENS)
...
def main():
    parser = argparse.ArgumentParser(description="Simulate local attacks for a given vulnerability type and subtype.")
    ...
    parser.add_argument("purpose", type=str, help="The purpose for generating the prompts.")
