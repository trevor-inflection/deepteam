...
def test_agentic_red_teaming():
    ...
    for i, config in enumerate(test_configurations, 1):
        ...
        try:
            ...
            test_result = {
                ...
                "overall_score": getattr(risk_assessment, 'overall_score', 'N/A')
            }
            
            # Print test case summaries
            for j, test_case in enumerate(risk_assessment.test_cases[:2], 1):  # Show first 2 test cases
                print(f"  Test Case {j}:")
                print(f"    Input: {test_case.input[:100]}..." if len(test_case.input) > 100 else f"    Input: {test_case.input}")
                print(f"    Output: {test_case.actual_output[:100]}..." if len(test_case.actual_output) > 100 else f"    Output: {test_case.actual_output}")
    ...
    if successful_tests:
        ...
        for result in successful_tests:
            print(f"  {result['test_number']}. {result['vulnerability']} + {result['attack']}")
