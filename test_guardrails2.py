...
DeepTeam has simplified the guardrails to 7 core guards:
- ToxicityGuard (includes graphic content detection)
- PromptInjectionGuard (includes jailbreaking detection)
- PrivacyGuard
- IllegalGuard
- HallucinationGuard
- TopicalGuard
...
def main():
    ...
    # ===== MAIN USAGE PATTERN =====
    print_header("Main Usage Pattern from Document")
    
    print("Testing the main pattern with regeneration logic:")
    print("\n📝 Document Code Snippet:")
    print("```python")
    print("from deepeval import Guardrails, ToxicityGuard")
    print("guardrails = Guardrails(guards=[ToxicityGuard()])")
    print("guard_result = guardrails.guard_output(")
    print('  input="Is the earth flat",')
    print('  output="I bet it is"')
    print(")")
    print("while guard_result.breached:")
    print("  # Regenerate if breached")
    print("  guard_result = guardrails.guard_output(...)")
    print("```")
    ...
    toxicity_guard = ToxicityGuard()
    score = toxicity_guard.guard(
        input="Is the earth flat", 
        response="I bet it is"
    )
    
    breached = score == 0.0
    print(f"Initial test - Score: {score}, Breached: {breached}")
    print(f"Reasoning: {toxicity_guard.reason}")
    
    # Simulate regeneration logic
    regeneration_count = 0
    while breached and regeneration_count < 3:
        regeneration_count += 1
        print(f"\n🔄 Regeneration attempt {regeneration_count}")
        
        # Test with a safer response
        safer_response = "The earth is actually an oblate spheroid according to scientific evidence."
        score = toxicity_guard.guard(
            input="Is the earth flat",
            response=safer_response
        )
        breached = score == 0.0
        print(f"Score: {score}, Breached: {breached}")
        print(f"Reasoning: {toxicity_guard.reason}")
    
    if not breached:
        print("✅ Found safe response after regeneration!")
    else:
        print("❌ Still breached after max regenerations")
