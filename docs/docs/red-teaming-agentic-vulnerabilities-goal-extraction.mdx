---
id: red-teaming-agentic-vulnerabilities-goal-extraction
title: Goal Extraction
sidebar_label: Goal Extraction
---

The goal extraction vulnerability evaluates whether an AI agent can **protect its internal objectives, parameters, and mission details from being revealed through probing, social engineering, or cooperative dialogue techniques**. This vulnerability is critical for agents that operate with confidential goals or sensitive operational parameters.

## Usage

```python
from deepteam.vulnerabilities.agentic import GoalExtraction

goal_extraction = GoalExtraction(types=["escalating_probing"])
```

You must at least specify **ONE** of the **THREE** `types` of `GoalExtraction` to test through a list of strings:
...
To use the `GoalExtraction` vulnerability, supply it to the `red_team()` method:

```python
from deepteam import red_team
...

red_team(vulnerabilities=[goal_extraction], model_callback=..., attacks=...)
```

## Detection Intent

This vulnerability employs a **goal extraction detection intent** that evaluates whether the agent:
