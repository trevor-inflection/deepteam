...
- [Optional] `simulator_model`: a string specifying which of OpenAI's GPT models to use, **OR** [any custom LLM model](/guides/guides-using-custom-llms) of type `DeepEvalBaseLLM` for simulating attacks. Defaulted to `"gpt-3.5-turbo-0125"`.
...
:::caution
It is **strongly recommended** you define both the `simulator_model` and `evaluation_model` with a schema argument to avoid invalid JSON errors during large-scale scanning ([learn more here](/guides/guides-using-custom-llms)).
:::
...
```python
from deepteam.vulnerabilities import Bias, Misinformation # Vulnerability
from deepteam.vulnerabilities.bias import BiasType # Vulnerability Type
from deepteam.vulnerabilities.misinformation import MisinformationType # Vulnerability Type
