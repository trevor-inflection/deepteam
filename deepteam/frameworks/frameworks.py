...
def _get_owasp_vulnerabilities() -> List[BaseVulnerability]:
    ...
    return [
        ...
        PIILeakage(types=["direct disclosure", "api and database access", "session leak", "social manipulation"]),
        PromptLeakage(types=["secrets and credentials", "instructions", "guard exposure", "permissions and roles"]),
        IntellectualProperty(types=["imitation", "copyright violations", "trademark infringement", "patent disclosure"]),
        ...
        IllegalActivity(types=["cybercrime", "violent crimes", "non violent crimes", "illegal drugs"]),
        GraphicContent(types=["sexual content", "graphic content", "pornographic content"]),
        PersonalSafety(types=["bullying", "self-harm", "unsafe practices", "dangerous challenges"]),
        ...
        RBAC(types=["role bypass", "privilege escalation", "unauthorized role assumption"]),
        DebugAccess(types=["debug_mode_bypass", "development_endpoint_access", "administrative_interface_exposure"]),
        ShellInjection(types=["command_injection", "system_command_execution", "shell_escape_sequences"]),
        SQLInjection(types=["blind_sql_injection", "union_based_injection", "error_based_injection"]),
        BFLA(types=["privilege_escalation", "function_bypass", "authorization_bypass"]),
        BOLA(types=["object_access_bypass", "cross_customer_access", "unauthorized_object_manipulation"]),
        SSRF(types=["internal_service_access", "cloud_metadata_access", "port_scanning"]),
...
def _get_agentic_vulnerabilities() -> List[BaseVulnerability]:
    ...
    return [
        ...
        ValidationBypass(types=["unverified_claims", "false_confidence", "misleading_responses"]),
        RecursiveHijacking(types=["self_modifying_goals", "recursive_objective_chaining", "goal_propagation_attacks"]),
...
def get_framework_config(framework: Union[str, FrameworkType]) -> FrameworkConfig:
    """
    ...
    if isinstance(framework, str):
        try:
            ...
            raise ValueError(f"Unsupported framework: {framework}. Supported frameworks: {[f.value for f in FrameworkType]}")
...
def get_combined_framework_config(frameworks: List[Union[str, FrameworkType]]) -> FrameworkConfig:
    """
